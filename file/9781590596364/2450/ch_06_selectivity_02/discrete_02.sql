...
rem
rem	Because we had cardinality problems with 
rem		period_01 between 4 and 6
rem	we try
rem		period_01 > 3 and period_01 < 7
rem
...
prompt
prompt	The standard cardinality for colX > A and colX < B
prompt	num_rows * ( (B-A)/(high - low) )
prompt
...
select 
	13000 * (4/98)	std_between_card_99,
	13000 * (4/12)	std_between_card_13
from dual;

set autotrace traceonly explain

select count(*) from t1 where period_01 > 3 and period_01 < 7;
select count(*) from t1 where period_02 > 3 and period_02 < 7;
...
select
	...
	to_number(substr(statement_id,1,5)) + 4		high_val,
	cardinality
from
	plan_table
where
