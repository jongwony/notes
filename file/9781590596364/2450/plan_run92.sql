...
rem	Then execute plan_run92.sql
rem
rem	Preparation:
rem	============
...
select
	...
	decode(cost,null,null,
		'Old Cost (' || cost || ',' || cardinality || ',' || bytes || ') ' 
	) ||
	decode(io_cost,null,null,
		' New Cost (' || io_cost || ',' || cpu_cost|| ',' || temp_space || ') '
	) ||
...
execute explain_plan92.plan_walk('&m_statement_id',0)
execute explain_plan92.plan_walk('&m_statement_id',1)
