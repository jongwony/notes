...
[NumPy 라이브러리]는 이런 문제에 사용될 수 있는  `matrix` 객체를 가집니다. 행렬은 [3.9장]() 에서 설명한 배열 객체와 유사하지만, 계산하기 위해서는 선형 대수 규칙을 따라야 합니다. 여기 매우 필요한 기능을 설명한 예제가 있습니다.
...
표시된 기능에 추가로 `random()`은 `uniform`, `Gaussian`, 그리고 다른 확률 분포을 위한 함수를 포함합니다. 예를 들어, `random.uniform()`은 균등분포(uniform distribution) 수를 계산하며, `random.gauss()`는 정규분포(normally distribution) 수를 계산합니다. 지원되는 다른 배포판에 대한 정보는 `documentation`을 참조하시기 바랍니다.
...
세부 상수 조절은 그래프를 보기 편하게 하기 위한 것이며 자세한 내용은 주석을 참고하시기 바랍니다.
