...
Python의 `datetime` 모듈은 다음과 같은 계산을 수행하는데 도음을 주기 위한 유틸리티 함수와 클래스를 가집니다. 이 문제에 대한 알맞은, 일반적인 해결책은 다음과 같습니다.
...
만일 이처럼 많은 날짜 계산을 수행하려면 대신 [python-dateutil 패키지]()를 설치하는 것이 나을 수 있습니다. 예를 들어 다음은 같은 계산을 `relativedelta()`함수와 `dateutil`를 사용하여 수행하는 예제입니다.
