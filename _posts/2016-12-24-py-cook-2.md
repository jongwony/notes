...
이를 알기위한 가장 중요한 것은 `+` 연산자를 사용하여 많은 문자열을 함께 결합하는 것은 메모리 복사와 `garbage collection`이 일어나기 때문에 매우 비효율 적이라는 것입니다.
...
`I/O operation`을 섞고 문자열을 `concatenation`하는 것은 애플리케이션에 따른 연구가 필요할 수 있습니다. 예를 들어 다음 두 코드를 고려하면
...
또는 I/O operation을 결합하는 것에 대한 영리한 `hybrid scheme`의 종류를 떠올릴 수 있습니다.
...
이런 종류의 분할을 수행하려면, 첫번째 단계는 모든 가능한 토큰을 `whitespace`를 포함하여, 다음과 같은 명명된  `capture group`을 사용하는 `regular expression` 패턴으로써 정의하는 것입니다.
...
`master regular expression`의 토큰의 순서 역시 문제입니다. 매칭 될 때 `re`는 지정된 순서에서 패턴 매치를 시도합니다. 그러므로 패턴이 긴 패턴의 `substring`이 될 경우 긴 패턴을 먼저 오도록 해야 합니다.
...
마지막으로 하지만 적어도 `substring` 형태의 해턴을 조심해야할 필요가 있습니다. 예를 들어 다음 두 패턴이 있다고 가정하면
