...
마지막으로 `CSV` 데이터를 읽는 목적이 데이터 분석 및 통계를 수행하는 것이라면 [Pandas package](//pandas.pydata.org/)를 살펴볼 수 있습니다. `Pandas`는 `CSV` 데이터를 `DataFrame` 객체로 로드하는 편리한 `pandas.read_csv()` 함수를 포함합니다. 거기서 다양한 요약 통계를 생성하고 데이터를 필터링하며 다른 종류의 높은 레벨의 작업을 수행할 수 있습니다. [6.13장]()에 예가 주어저 있습니다.
...
이 형테로 데이터가 주어지면 [PEP 249]()에 설명된 것처럼 Python의 표준 데이터베이스 `API`를 사용하여 관계 데이터베이스와 상호작용하는 것이 비교적 간단합니다. `API`의 요지는 데이터베이스에 대한 모든 작업이 `SQL` 쿼리에 의해 수행된다는 것입니다. 각 입력 또는 출력 데이터의 열은 `tuple`로 표현됩니다.
...
[SQLAlchemy]()와 같은 라이브러리는 Python 클래스로써 데이터베이스 테이블을 설명할 수 있도록 허용하고 데이터베이스 작업이 대부분 기본적인 `SQL` 대부분을 숨기는 동안 데이터베이스 연산을 수행될 수 있도록 합니다.
