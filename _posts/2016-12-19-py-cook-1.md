...
<div id='index-table'>
<h2>1. Data Structures and Algorithms</h2>
</div>
...
항목을 검색하기 위한 코드를 작성할 때는 위에서 보여진 Solution 대로 일반적으로 `yield`문이 포함된 제너레이터 함수를 사용합니다. 이렇게 검색 프로세스가 반환되는 결과를 사용하는 코드로부터 분리됩니다.
...
`OrderedDict`는 나중에 서로 다른 형식으로 serialize 또는 인코딩하고 싶은 `mapping`을 만들 때 특히 유용합니다. 예를 들어 JSON 인코딩에 나타나는 필드의 순서를 정밀하게 제어하려면 먼저 `OrderedDict`에서 데이터를 만드는 것이 트릭이 될 것입니다.
...
두 `dictionary`에서 같은 `key`와 같은 `value` 등의 공통적인 부분을 찾아야 합니다.
...
마지막으로, 하지만 적어도 만약 목표가 다양한 인스턴스의 속성을 바꾸기 위한 효율적인 자료 구조를 정의하는 것이라면 `namedtuple`은 가장 좋은 선택은 아니라는 것을 아셔야 합니다. 대신에 클래스를 정의할 때 `__slots__`를 사용하는 것을 고려하시기 바랍니다. [8.4장 참조](/2017/02/24/py-cook-8.html#84-saving-memory-when-creating-a-large-number-of-instances)
