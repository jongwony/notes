...
<div class='warn'>
이 문서는 Python Coobook 3rd edition - O'REILLY, David Beazley & Brian K. jones 를 참고했습니다.<br>
개인적으로 공부한 내용이라 오역이 있을 수 있으며 <a href="//wikidocs.net/book/1">Python을 완전히 처음 접하는 경우</a>에는 적합하지 않습니다.
</div>
...
## 1.14 Native Comparison 지원 없이 객체를 정렬
...
`compress()` 함수를 사용함으로써 보통 두 `sequence`의 인덱스를 추출하여 일일이 대입 연산자를 통해 대응시키는 방법으로 고민을 많이 했었던 저에게는 가려운 부분을 시원하게 긁는 파트였습니다.