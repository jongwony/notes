...
마지막으로 [8.25장](/2017/02/24/py-cook-8.html#825-creating-cached-instances)에 설명한 대로 캐시된 인스턴스를 만든다고 가정해 보겠습니다. 다음은 그것을 구현하는 메타클래스입니다.
...
다음은 함수 시그니처 적용에 대한 보다 구체적인 예입니다.
...
메타클래스의 핵심 기능은 정의할 시에 클래스의 내용을 검사할 수 있다는 것입니다.
...
이 장에서 사용된 기술의 한가지 중요한 점은 메타클래스를 적절하게 지원한다는 것입니다.
...
솔직히 현실에 코드를 적용할 수 있도록 이 장에 너무 많은 기교가 적용될 수 있습니다.
하지만 메타클래스 및 *descriptor*의 내부 동작에 대해 살펴보고 일부 개념을 강화할 수 있습니다.
따라서 이 장을 직접 적용하지 않더라도 기본적인 아이디어 중 일부는 메타클래스, *descriptor*, 함수 *annotation*과 관련된 다른 프로그래밍 기술에 영향을 미칠 수도 있습니다.

구현의 메인 아이디어는 상대적으로 간단합니다.
`MultipleMeta` 메타클래스는 `__prepare__()` 메서드를 사용하여 커스텀 클래스 *dictionary*를 `MultiDict`의 인스턴스로 제공합니다.
일반적인 *dictionary*와 달리 `MultiDict`는 항목이 설정될 때 엔트리가 이미 존재하는지 체크합니다.
그런 경우 중복되는 엔트리는 `MultiMethod`의 인스턴스 내부로 함께 병합됩니다.

`MultiMethod`의 인스턴스는 타입 시그니처에서 함수에 매핑을 만들어 메서드를 수집합니다.
이 구성을 하는 동안 함수 *annotation*을 사용하여 이러한 시그널을 수집하고 매핑을 만듭니다.
이는 `MultiMethod.register()` 메서드에서 수행됩니다.
이 매핑의 한가지 중요한 부분 중 하나는 다중메서드의 경우 모든 인수에 타입을 지정하지 않으면 오류가 발생합니다.

`MultiMethod` 인스턴스가 *callable* 객체를 에뮬레이션하도록 하려면 `__call__()` 메서드가 구현됩니다.
이 메서드는 `self`를 제외한 모든 인수에서 타입 *tuple*을 만들고 내부 맵에서 메서드를 찾아 적절한 메서드를 호출합니다.
`MultiMethod` 인스턴스가 클래스 정의 내부에서 올바르게 동작하도록 하려면 `__get__()`이 필요합니다.
구현에서 적절한 바운드 메서드를 만드는 데 사용됩니다.
...
확실히, 이 장에서 움직이는 많은 부분이 있습니다. 하지만 약간 불행하게도 많은 한계가 있다는 것도 고려해야 합니다.
그 중 하나로 솔루션은 다음 키워드 인수를 처리하지 못합니다.
...
이런 지원을 추가할 수 있는 방법이 있을 수 있지만, 메서드 매핑에 완전히 다른 접근이 필요합니다.
문제는 키워드 인수가 특정 순서로 도달하지 않는다는 점입니다.
위치 인수가 섞여 있을 때 `__call__()` 메서드에서 어떻게든 정렬해야하는 인수가 혼란에 빠지게 됩니다.

이 장은 상속에 대한 지원도 심각하게 제한됩니다.
예를 들어 다음과 같은 동작이 이루어지지 않습니다.
...
실패하는 이유는 `x:A` *annotation*이 서브클래스(`B`의 인스턴스 같은) 인스턴스와 일치하지 않기 때문입니다.
...
메타클래스 및 *annotation*을 사용하는 대신 *decorator*를 사용하여 비슷한 방법을 구현할 수도 있습니다.
...
*decorator* 버전을 사용하면 다음과 같이 코드를 작성할 수 있습니다.
...
*decorator* 솔루션은 이전 구현과 마찬가지로 동일한 제한을 겪고 있습니다(즉, 키워드 인수 및 깨진 상속에 대한 지원이 없습니다).

모든게 같은 결과가 되므로 일반적으로 범용 코드로 다중 디스패치를 피하는 것이 가장 좋습니다.
어떤 종류의 패턴 일치를 기반으로 메서드를 디스패칭하는 프로그램에서와 같이 특별한 경우가 있을 수도 있습니다.
예를 들어 [8.21장](/2017/02/24/py-cook-8.html#821-implementing-the-visitor-pattern)에 설명된 *visitor* 패턴은 어떤 방법으로 다중 디스패치를 사용하는 클래스로 다시 작성될 수 있습니다.
하지만 그런 방법이 아니라면 더 간단한 접근을 고수하는 것은 결코 잘못된 생각이 아닙니다(다른 이름을 가진 메서드를 사용하기만 하면 됩니다).

다중 디스패치를 구현하는 여러가지 방법에 관한 아이디어가 Python 커뮤니티에서 몇년간 떠올랐습니다.
토론을 시작하기에 좋은 시점으로 Guido van Rossum의 블로그 게시물인 [Five-Minute Multimethods in Python](//www.artima.com/weblogs/viewpost.jsp?thread=101605)을 참조하시기 바랍니다.

## 9.21. Avoiding Repetitive Property Methods

#### Problem

타입 체킹 같은 일반적인 작업을 수행하는 *property* 메서드를 반복적으로 정의하는 클래스를 작성하고 있습니다.
많은 코드 반복이 없도록 코드를 단순화 하려고 합니다.

#### Solution

*property* 메서드에 의해 wrap 되는 속성의 간단한 클래스를 살펴봅니다.
...
class Person:
    ...
    @age.setter
    def age(self, value):
        if not isinatance(value, int):
            raise TypeError('age must be an int')
        self._age = value
```

보시다시피, 많은 코드가 단순히 속성 값에 대한 일부 타입 *assertion*을 시행하기 위해 작성되고 있습니다.
이런 코드를 볼 때마다 이를 단순화 하기 위한 다른 방법을 찾는 것이 좋습니다.
한가지 가능한 접근은 단순히 *property*만을 정의하고 반환하는 함수를 만드는 것입니다.
...
이 장은 중요한 내부 함수 또는 *closure*의 기능을 설명합니다. 즉, 이들로 매크로처럼 동작하는 많은 코드를 작성하는 데 사용합니다.
이 예제의 `typed_property()` 함수는 약간 이상해 보일 수 있지만 실제로 *property* 코드를 생성하고 *property* 객체를 반환합니다.
따라서 클래스에서 사용할 때 `typed_property()` 내부에 나타나는 코드가 클래스 정의 자체에 배치된 것처럼 정확하게 동작합니다.
심지어 *property*의 *getter*, *setter* 메서드가 `name`, `expected_type`, `storage_name` 같은 로컬 변수에 액세스 하는 경우에도 이런 변수는 *closure* 뒤에서 유지됩니다.

이 장은 `functools.partial()` 함수를 사용하여 흥미로운 방식으로 조정할 수 있습니다.
...
위의 코드는 [8.13장](/2017/02/24/py-cook-8.html#813-implementing-a-data-model-or-type-system)에 나와있는 타입 시스템 *descriptor* 코드 중 일부처럼 보이기 시작할 것입니다.

## 9.22. Defining Context Managers the Easy Way

#### Problem

`with` 문을 사용하여 새로운 종류의 컨텍스트 매니저를 구현하려고 합니다.

#### Solution

가장 단순한 방법 중 하나는 `contextlib` 모듈의 `@contextmanager` *decorator*를 사용하여 새로운 컨텍스트 매니저를 작성하는 것입니다.
다음은 코드 블록을 실행할 시 컨텍스트 매니저의 예입니다.
...
`timethis()` 함수에서 `yield` 이전의 모든 코드는 컨텍스트 매니저의 `__enter__()` 메서드로 실행됩니다.
`yield` 뒤의 모든 코드는 `__exit__()` 메서드로 실행됩니다.
예외가 발생할 경우 `yield` 문에서 일어나게 됩니다.

다음은 *list* 객체의 트랜잭션 종류를 구현하는 약간 더 심화된 컨텍스트 매니저입니다.
...
여기서의 아이디어는 전체 코드 블록이 예외 없이 완료될 때 까지만 *list*에 대한 변경사항이 적용된다는 것입니다.
다음은 이를 설명하는 예제입니다.
...
일반적으로 컨텍스트 매니저를 작성하기 위해 `__enter__()`와 `__exit__()` 메서드와 함께 클래스를 정의합니다.
...
비록 이것도 어렵진 않지만 `@contextmanager`를 사용하여 간단한 함수를 작성하는 것 보다는 더 지루(tedious)합니다.

`@contextmanager`는 실제로 자체 포함된 컨텍스트 매니저 함수를 작성하는 데만 사용됩니다.
만약 `with` 문을 지원해야 하는 객체(예: 파일, 네트워크 연결, lock)가 있다면
여전히 `__enter__()`와 `__exit__()` 메서드로 각각 구현해야 합니다.

## 9.23. Executing Code with Local Side Effects

#### Problem

`exec()`을 사용하여 *caller scope*에서 코드를 실행하지만 실행 후 아무런 결과도 보이지 않습니다.

#### Solution

문제를 더 잘 이해하기 위해 간단한 실험을 합니다.
먼저 글로벌 네임스페이스에서 코드를 실행합니다.
...
보시다시피 마치 `exec()` 문이 실제로 전혀 실행되지 않은 것처럼 `NameError`가 발생합니다.
이는 나중에 계산에서 `exec()`의 결과를 사용하려는 경우 문제가 될 수 있습니다.

이런 종류의 문제를 해결하기 위해 `exec()` 호출 이전에 로컬 변수의 *dictionary*를 얻기 위한 `locals()` 함수를 사용해야 합니다.
그 직후에 수정된 값을 `locals` *dictionary*로부터 추출할 수 있습니다.
...
`exec()`의 정확한 사용은 실제로 꽤 까다롭습니다. 사실 `exec()`의 사용을 고려하는 대부분의 상황에서, 더 우아한 솔루션이 존재합니다(*decorator*, *closure*, 메타클래스 등).

하지만 그래도 꼭 `exec()`를 사용해야 한다면 이 장은 올바르게 사용하는 미묘한 측면에 대해 살펴봅니다.
기본적으로 `exec()`는 로컬 및 글로벌 *caller* 스코프에서 코드가 실행됩니다.
하지만 함수 내부에서 `exec()`에 전달된 로컬 스코프는 실제 로컬 변수의 복사본입니다.
따라서 `exec()`에서의 코드가 어떤 종류의 수정을 일으키면 그 수정은 실제로 로컬변수에 전혀 반영되지 않습니다. 다음은 이 효과를 보여주는 또 다른 예제입니다.
...
로컬 변수를 얻기 위해 `locals()`를 호출할 때 위 솔루션에서 보였듯이 `exec()`로 전달된 `locals`의 복사본을 얻습니다.
실행 후 *dictionary*의 값을 검사함으로써 수정된 값을 얻을 수 있습니다.
다음은 그것을 보여주는 실험입니다.
...
마지막 단계의 출력을 주의깊게 살펴보시기 바랍니다.
`loc`으로부터 수정된 값을 다시 `x`로 복사하지 않으면 변수는 변경되지 않고 유지됩니다.

`locals()`의 사용에서 작업의 순서에 대해 조심해야 합니다.
`locals()`가 호출될 때마다 지역 변수의 현재 값을 취하여 *dictionary*의 해당 항목을 덮어 씁니다.
이 실험으로 결과를 살펴봅니다.
...
`locals()`에 대한 마지막 호출이 `x`를 덮어 쓰게 된 것을 확인합니다.

`locals()` 사용의 대안으로서, 별도로 자신만의 *dictionary*를 생성하고 `exec()`로 전달합니다.
...
대부분의 `exec()` 사용에서 이는 좋은 연습이 될 것입니다.
단지 글로벌 및 로컬 *dictionary*를 실행된 코드에 액세스할 이름으로 적절히 초기화하면 됩니다.

마지막으로 `exec()`를 사용하기 전에 다른 가능한 대안이 있는지 확인하시기 바랍니다. `exec()`의 사용을 고려하는 많은 문제들은 *closure*, *decorator*, 메타클래스 또는 다른 메타프로그래밍 기능으로 대체될 수 있습니다.

## 9.24. Parsing and Analyzing Python Source

#### Problem

Python 소스 코드를 파싱 및 분석하는 프로그램을 작성하려고 합니다.

#### Solution

대부분의 프로그래머는 Python이 문자열 형태로 제공된 소스코드를 계산하거나 실행할 수 있다는 것을 알고 있습니다.
...
하지만 `ast` 모듈은 Python 코드를 분석할 수 있는 추상 문법 트리(AST: abstract syntax tree)를 컴파일 하는 데 사용될 수 있습니다.
...
>>> top
<_ast.Module object at 0x1007473d0>
>>> ast.dump(top)
"Module(body=[For(target=Name(id='i', ctx=Store()), iter=Call(func=Name(id='range', ctx=Load()), args=[Num(n=10)], keywords=[], starargs=None, kwargs=None), body=[Expr(value=Call(func=Name(id='print', ctx=Load()), args=[Name(id='i', ctx=Load())], keywords=[], starargs=None, kwargs=None))], orelse=[])])"
```

소스 트리를 분석하려면 약간의 연구가 필요하지만 이는 AST 노드의 컬렉션으로 구성됩니다.
이 노드들을 동작하기 위한 가장 쉬운 방법은 `NodeName()`이 관심있는 노드에 매치하는 다양한 `visit_NodeName()` 메서드를 구현하는 *visitor* 클래스를 정의하는 것입니다.
다음은 로드, 저장, 삭제되는 이름에 대한 정보를 기록하는 클래스의 예제입니다.
...
class CodeAnalyzer(ast.NodeVisitor):
    ...
    def visit_Name(self, node):
        ...
        elif isinstance(node.stx, ast.Del):
...
마지막으로 `compile()` 함수를 사용하여 AST를 컴파일하고 실행할 수 있습니다.
...
소스코드를 분석하고 그로부터 정보를 얻을 수 있다는 사실은 다양한 코드 분석, 최적화 또는 검증 도구를 작성하는 시작일 수 있습니다.
예를 들어 `exec()`같은 함수로 맹목적으로 코드 일부를 전달하는 대신, 먼저 AST로 변환하고 어떤 일을 하는지 자세히 살펴볼 수 있습니다.
또한 모듈의 전체 소스코드를 보고 이를 통해 정적 분석을 수행하는 도구를 작성할 수 있습니다.

스스로 하는 일을 실제 알고 있는 경우 새로운 코드를 나타내기 위해 AST를 다시 작성할 수도 있습니다.
다음은 함수 본체의 소스코드를 다시 파싱하고 함수의 코드 객체를 재생성함으로써 글로벌로 액세스한 이름을 함수 본문으로 내리는 *decorator*의 예제입니다.
...
이 코드를 사용하면 다음과 같은 코드를 작성할 수 있게 됩니다.
...
*decorator*는 `countdown()` 소스코드를 다음과 같이 재작성합니다.
...
성능 테스트에서 함수를 20% 빠르게 해줍니다.

그렇다고 이제 이 *decorator*를 모든 기능에 적용하는 것은 아닙니다.
하지만 이것은 AST 조작, 소스 코드 조작 및 기타 테크닉을 통해 가능할 수 있는 매우 진보적인 것들을 잘 설명해줍니다.

이 장은 [ActiveState](//code.activestate.com/recipes/277940-decorator-for-bindingconstants-at-compile-time/)에서 Python의 바이트 코드를 조작하여 작동하는 비슷한 방법에서 영감을 얻었습니다.
AST로 작업하는 것은 좀 더 단순한 높은 수준의 접근 방식입니다.
다음 장에서 바이트 코드에 대한 더 많은 정보를 참고하시기 바랍니다.

## 9.25. Disassembling Python Byte Code

#### Problem

인터프리터가 사용하는 낮은 레벨의 바이트 코드로 디스어셈블링하여 코드가 무엇을 하는지 더 자세하게 알아보려고 합니다.

#### Solution

`dis` 모듈은 모든 Python 함수의 디스어셈블리를 출력하는 데 사용될 수 있습니다.
...
`dis` 모듈은 프로그램이 매우 낮은 레벨에서 어떤 일이 일어나는지 연구해야 할 필요가 있을 경우 유용할 수 있습니다(예를 들어 성능 특성을 이해하려는 경우).

`dis()` 함수에 의해 해석된 *raw* 바이트 코드는 다음과 같은 함수에서 사용가능합니다.
...
이 코드를 직접 해석하고 싶다면 `opcode` 모듈에 정의된 상수의 일부를 사용해야 합니다.
...
아이러니하게도 `dis` 모듈에는 프로그래밍 방식으로 바이트 코드를 처리하는 쉬운 함수가 없습니다.
하지만 이 *generator* 함수는 *raw* 바이트 코드 시퀀스를 취해 opcode와 인수로 변환시켜 줄 것입니다.
...
이 함수를 사용하면 다음과 같은 코드를 사용할 수 있습니다.
...
거의 알려진 사실은 아니지만, 원하는 함수의 모든 *raw* 바이트 코드를 바꿀 수 있습니다.
이 작업을 수행하는 데 약간의 작업이 필요하지만 여기 관련된 예가 있습니다.
...
>>> # Make a compleetely new code object with bogus byte code
>>> import types
>>> newbytecode = b'xxxxxxx'
>>> nc = types.CodeType(c.co_argcount, c.co_kwonlyargcount,
...     c.co_nlocals, c.co_stacksize, c.co_flags, newbytecode,
...     c.co_consts, c.co_names, c.co_varnames, c.co_vilename,
...
인터프리터가 충돌하는 것은 이런 위험한 일의 결과일 것입니다.
하지만 고급 최적화 및 메타프로그래밍 도구로 작업하는 개발자는 실제 바이트 코드를 다시 작성하려는 경향이 있습니다. 이 마지막 부분으로 어떻게 하는지 보여줍니다.
[ActiveState의 코드](//code.activestate.com/recipes/277940-decorator-for-bindingconstants-at-compile-time/)에서 동작하는 또 다른 코드의 예제를 참고하시기 바랍니다.